function [M,Q]=community_louvain_EJC(W,gamma,M0,B)
...
if strcmp(type_B,'negative_sym') || strcmp(type_B,'negative_asym') || strcmp(type_B,'negative_sym_CPM') || strcmp(type_B,'negative_asym_CPM')
...
%%%%%%% EJC New 1-18-19 %%%%%%%
if strcmp(type_B,'negative_sym_CPM') || strcmp(type_B,'negative_asym_CPM')
    % compute positive and negative weights separately, according to
    % Rubinov and Sporns 2011 ('Weight conserving...')
    W0 = W.*(W>0);                          %positive weights matrix
    s0 = sum(sum(W0));                      %weight of positive links
    Pij = ones(length(W))*mean(mean(W));     %null model of uniform mean wts
    B0 = W0-gamma*(Pij)/s0;                    %positive modularity
...
if type_B
    switch type_B
        ...
        case 'negative_sym_CPM';    B = B0/(s0+s1) - B1/(s0+s1);
        case 'negative_asym';   B = B0/s0      - B1/(s0+s1);
        case 'negative_asym_CPM'; B = B0/s0      - B1/(s0+s1);
