...
for i=1:npart
	disp(['partition ',num2str(i)]) % added by EJC on 1/22/19 to track progress
    for j=(i+1):npart
        pairwise_simm(i,j) = zrand_EJC(C(i,:),C(j,:)); % added by EJC on 1/22/19 to speed up computation by eliminating unnecessary steps
    end
end
