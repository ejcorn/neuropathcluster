...
p.acc <- plot.model.perf.met(met=met,perf.met='Test Accuracy',ttl='',colors = clusterColors)
p.roc <- plot.model.roc(met=cluster.res,ttl='',colors=clusterColors)
p.all <- plot_grid(plotlist = 
	list(p.sen,remove.y.ticklabels(p.spec),remove.y.ticklabels(p.auc),remove.y.ticklabels(p.acc),p.roc),
