...
rel_widths <- c(1,1)
w.multiplier <- 1
if(grepl('CSFGene',extralab) | grepl('GeneOnly',extralab)){w.multiplier <- 2} # make plot wider if both csf and gene betas 
if(grepl('GeneOnly',extralab) & grepl('MMSE',extralab)){rel_widths <- c(0.6,1)}

p1 <- plot.featureweights.lm.contintsplit(cluster.res,df,clusterColors,rel_widths = rel_widths)
...
if(grepl('CSFOnly',extralab) | grepl('CSFGene',extralab)){
	...
	rel_widths = c(1,1) # specify ratio of plot width for continuous to interval predictors, if present
}
if(grepl('GeneOnly',extralab)){
	...
	rel_widths <- c(1,1) # specify ratio of plot width for continuous to interval predictors, if present
	# if you have gene only  and MMSE, then you have 1 continuous predictor and several interval predictors so scale accordinglt
	if(grepl('MMSE',extralab)){rel_widths <- c(0.5,1)}
}

p2 <- plot.featureweights.lm.contintsplit(dz.res,df,dz.colors,rel_widths = rel_widths)
