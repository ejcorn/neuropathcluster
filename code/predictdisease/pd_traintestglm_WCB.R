...
save(dz.res,df,WCB.classes, file = paste(savedir.out,'predictdz_WCB_GLMperf',extralab,'.RData',sep=''))

df$WCB_LABELS <- row.Which.Max.tie(clusters)
cluster.res <- kfold.GLM(x=df,y=clusters,k.folds=3,nreps=100)
save(cluster.res,df, file = paste(savedir.out,'predictcluster_WCB_GLMperf',extralab,'.RData',sep=''))
