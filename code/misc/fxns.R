...
exclude.dz <- function(patientSample,dz.exc,n.dx){
  ...
  # if ABC score cannot be determined, we use NPDx Likelihood (subjective pathologist dx as far as I can tell)
  # instead. 
  ...
  if(dz.exc == 'Alzheimer\'s disease'){
    ...
    Rem.Mask <- rep(NA,nrow(patientSample)) # initialize mask to select patients without ABC stage intermediate-high ADNC
    Rem.Mask[patientSample$ADStatus == 'Low' | patientSample$ADStatus == 'None'] <- TRUE
    Rem.Mask[patientSample$ADStatus == 'Intermediate'| patientSample$ADStatus == 'High']  <- FALSE # if patients have high or intermediate ABC AD
    
    # Now all other remaining patients are NAs because they have missing scores
    # in these patients we will diagnose them with AD based on NPDx1-n.dx having > low probability
...
insert.ABC <- function(patientSample,ABC){
  # this function takes the logical opposite of the mask generated by exclude.dz, specifically for AD, and 
  # replaces all Alzheimer's mentions in NPDx to correspond only to ABC stage intermediate-high levels of AD neuropathologic change
  # according to Montine et al. 2012 table 3 ADNPC ABC staging found in data/
  # this involves:
  # - removing AD diagnoses when patients don't meet ABC criteria
  # - shifting other diagnoses up, i.e. if you delete NPDx1, make NPDx2 the new NPDx1
  # - adding AD diagnoses to NPDx1 if patients meet ABC criteria
  ...
  # allow for new likelihood level "ABC"
  #levels(patientSample$NPDx1Likelihood) <- c(levels(patientSample$NPDx1Likelihood),'ABC')
  ...
  for(i in 1:nrow(patientSample)){ # iterate through each patient
    ...
    if(ABC[i] & is_empty(AD.idx)){
      # add AD diagnoses to NPDx1 when patients meet ABC...shift other dx's back
      ...
      patientSample[i,'NPDx1Likelihood'] <- 'ABC Intermediate-High'
    ...
    } else if(ABC[i] & !is_empty(AD.idx)){
      # move existing AD diagnoses to NPDx1 if ABC stage int-high
      ...
      # if no diagnosis of AD and doesn't meet ABC criteria just print
