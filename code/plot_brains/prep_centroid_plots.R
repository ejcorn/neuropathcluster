...
pathItems.type <- list("NeuronLoss","Gliosis","Angiopathy","Thio","TDP43","Tau","Syn")
...
for(C in rownames(centroids)){
	...
	for(p.type in names(centroid.by.type)){
		...
		C.region.plot.t['DG',1] <- mean(c(C.region.plot.t['DG',1],C.region.plot.t['CS',1]),na.rm=T)		
...
system(paste0('tar -zcf ',params$sourcedata.dir,'FigS8a_SourceData_SurfacePlots.tar.gz ',savedir))