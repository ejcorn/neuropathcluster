...
Rem.Mask <- !exclude.dz(patientSample[INDDIDs %in% unique(Genes$INDDID),],dz.iso,n.dx)
...
print(paste('Genes: Isolate',dz.iso))
...
list[clusterColors,clusterNames] <- 
  lapply(list(clusterColors,clusterNames), function(X) X.n.exclude(X,cluster.counts,n=20))
...
for(g.i in names(Allele.Tables)){
  ...
  A <- A[Rem.Cl,] # apply mask to exclude clusters with too few observations
  results[[g.i]] <- list()
  ...
  for(k.1 in clusterNames){
    ...
    for(k.2 in clusterNames){
      ...
      # -1 drops first column, which is intercept allele
...
for(g.i in names(betas)){
  ...
  for(a.i in names(betas[[g.i]])){
    diag(betas[[g.i]][[a.i]]) <- 0 # set NAs to 0
  ...
  ggsave(filename = paste(savedir,g.i,'BetasIsolate',dz.iso,'NDx',n.dx,'.pdf',sep=''),plot = p.all,
