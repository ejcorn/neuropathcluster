...
for(g.i in names(betas)){
  ...
  for(a.i in names(betas[[g.i]])){
    ...
    if(dz.exc == 'Alzheimer\'s disease'){save(bmat,pmat,file = paste(params$sourcedata.dir,'FigS14c-e_SourceData_',g.i,'-',a.i,'Exclude',dz.exc,'From',paste(exc.cl,collapse=','),'.RData',sep=''))}
    if(dz.exc == 'PSP'){save(bmat,pmat,file = paste(params$sourcedata.dir,'FigS15a-b_SourceData_',g.i,'-',a.i,'Exclude',dz.exc,'From',paste(exc.cl,collapse=','),'.RData',sep=''))}
  }
