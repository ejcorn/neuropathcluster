...
Rem.Mask <- !exclude.dz(patientSample[INDDIDs %in% unique(cog$INDDID),],dz.iso,n.dx)
...
print(paste('Cog: Exclude',dz.iso))
print(cluster.counts)
print(paste('Exclude',dz.iso,'n =',length(partitionSample)))

# exclude clusters with < 2 members after Rem.Mask application
Rem.Cl <- cluster.exclude.mask(cluster.counts,partitionSample,n=2)
...
cluster.counts <- cluster.count(partitionSample,k)
print(paste('Cog: Isolate',dz.iso))
print(cluster.counts)
...
pdf(file = paste0(savedir,'/OverallMoCABoxplotsbyClusterLouvainIsolate',dz.iso,'NDx',n.dx,'.pdf'),height = 4.5/2.54,width=4.5/2.54,useDingbats = F)
...
p <- mult.comp.ggpubr(p)
pdf.options(reset = TRUE, onefile = FALSE)
pdf(file = paste0(savedir,'/MOCASubscoreBoxplotsbyClusterLouvainIsolate',dz.iso,'NDx',n.dx,'.pdf'),height = unit(4,'in'),width=unit(3.5,'in'),useDingbats = F)
plot(ggplot_gtable(p))
dev.off()
